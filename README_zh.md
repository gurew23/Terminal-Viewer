...
|**play_video**|lst:**tuple[tuple[tuple[tuple[int,int,int,int,int,int]]]]**, char:**str** = **_char**, fps:**int** = **30**|`lst`对应**多帧图像**，`fps`是帧率，默认30fps，无返回|**None**|
|**show_photo**|lst:**tuple[tuple[tuple[int,int,int,int,int,int]]]**, char:**str** = **_char**|同`parse`函数|**None**|
|**save**|lst:**tuple[tuple[tuple[int,int,int,int,int,int]]]** \| **tuple[tuple[tuple[tuple[int,int,int,int,int,int]]]]**,path:**str**,level:**int** = **3**|将`lst`序列化并`level`级压缩后保存到`path`|**None**|
...
|**myprint**|text:**str**|将`text`写入缓冲区后立即刷新|**None**|
